...
class LobbyService {
    ...
    private canPlayerPlayAnyCard(playerInfo: PlayerInfo, cards: Card[]): boolean {
        ...
        if(playerInfo){
            var cardsCombination = this.cardTable.hasValidCombination(playerInfo.getAllCards(), playerInfo, this.gameType);
            
            if(cardsCombination == null || cardsCombination.length == 0)
                return false;
            else
                return true
        }
